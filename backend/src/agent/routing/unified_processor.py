...
class UnifiedProcessor:
    ...
    async def process_message(
        ...
        try:
            ...
            if routing["system"] == "simple":
                result = await self._process_simple(message, files, conversation_id)
            elif routing["system"] == "advanced":
                result = await self._process_advanced(message, files, user_params, user_id, conversation_id)
            else:  # hybrid
                result = await self._process_hybrid(message, files, user_params, user_id, conversation_id)
    ...
    async def _publish_event(self, conversation_id: str, event_data: Dict):
    ...
    async def _process_simple(self, message: str, files: List, conversation_id: str) -> Dict[str, Any]:
