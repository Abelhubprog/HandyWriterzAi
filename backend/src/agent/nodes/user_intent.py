...
class UserIntentNode(BaseNode, BaseAgent):
    ...
    async def receive_input(self, state: HandyWriterzState) -> Dict[str, Any]:
        ...
        """
        # Extract all relevant input data from state
        input_data = {
            "auth_token": state.get("auth_token"),
            "uploaded_files": state.get("uploaded_files", []),
            "user_params": state.get("user_params", {}),
            "payment_transaction_id": state.get("payment_transaction_id")
        }

        # Validate required inputs
        if not input_data["auth_token"]:
    ...
    async def process(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        # Create a temporary state object for processing
        # In a real implementation, we would process the input_data directly
        # For now, we'll keep the existing logic but adapt it to the new interface

        # Since the existing methods expect a state object, we'll create a minimal one
        # This is a temporary solution - in a full refactor, we'd restructure the methods
        class MinimalState:
        ...
        temp_state = MinimalState(input_data)

        try:
            # Step 1: Validate authentication and payment
            auth_result = await self._validate_authentication(temp_state)
            if not auth_result["authenticated"]:
                raise ValueError("User authentication failed")

            # Step 2: Process file uploads
            uploaded_chunks = await self._process_file_uploads(temp_state)

            # Step 3: Extract and validate user parameters
            user_params = self._extract_user_parameters(temp_state)

            # Step 4: Calculate pricing and verify payment
            pricing_info = self._calculate_pricing(user_params)
            payment_verified = await self._verify_payment(temp_state, pricing_info)

            # Step 5: Store processed data (this would be handled in emit_output in a full refactor)
            await self._store_user_data(temp_state, user_params, uploaded_chunks)
    ...
    async def emit_output(self, processed_data: Dict[str, Any], state: HandyWriterzState) -> Dict[str, Any]:
        ...
        """
        auth_result = processed_data["auth_result"]
        user_params = processed_data["user_params"]
        uploaded_chunks = processed_data["uploaded_chunks"]
        payment_verified = processed_data["payment_verified"]
        pricing_info = processed_data["pricing_info"]
    ...
    def _broadcast_progress(self, state: HandyWriterzState, message: str, progress: float, error: bool = False):
        conversation_id = state.get("conversation_id")
        node_name = self.name
        event_type = "error" if error else "progress"
        if conversation_id:
            import asyncio
            _sse = get_sse_service()
            asyncio.create_task(_sse.publish_event(conversation_id, event_type, {"node": node_name, "phase": "init", "message": message, "progress": progress}))
